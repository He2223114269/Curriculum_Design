...
def main():
    ...
    """
    file = "cora/cora_cites"
    kind = "csv"
    layers = [2708,256,64,7] #[2708-256-64-7]
    ...
    labels_true = pd.read_csv("../input/cora/cora_content.csv", header=None)
    labels_pred = pd.read_csv("../output/" + file + "membership." + kind, header=None)
    labels_true = list(labels_true[1435])
