...
class DANMF(object):
    ...
    def save_membership(self):
        ...
        """
        # 使用 numpy 库的 argmax 函数找到每个数据点的概率最高的群集；
        # 将生成的数组转化为一个字典，其中每个数据点的索引对应于赋予其的群集编号；
        # 将字典写入类实例的 membership 成员变量中，并把它写入到一个指定路径的 JSON 文件中
        # 这个函数的工作可以看作是将聚类后的结果进行序列化，并保存到文件系统上，以便下次使用时重新使用这些聚类结果。
    ...
    def save_membership_to_csv(self):
        """
        Save cluster membership to CSV file.
        """
        with open(self.args.membership_path + ".csv", "w", newline='') as f:
            writer = csv.writer(f)
            # writer.writerow(["Data Point Index", "Cluster Index"])
            index = np.argmax(self.P, axis=1)
            self.membership = {int(i): int(index[i]) for i in range(len(index))}
            for i, cluster_id in self.membership.items():
